...
class YadroParser:
    ...
    def parse_program(self) -> Program:
        ...
        statements: List[Statement] = []
        while True:
            # Skip any whitespace tokens before checking for directives
            while self.match_any(TokenType.NEWLINE, TokenType.DEDENT):
                continue
            if self.check(TokenType.EOF):
                break
            # Check if the next token is a directive
            if self.match_any(
                ...
                break  # No more directives, start parsing statements
        while not self.check(TokenType.EOF):
            ...
            try:
                ...
                # If we get a parse error and we're at EOF, break gracefully
                if self.check(TokenType.EOF):
                    break
                raise
    ...
    def parse_directive(self) -> Directive:
        # Check the previous token since match_any already consumed the directive
        prev_token = self.previous()
        if prev_token.type == TokenType.DIRECTIVE_TARGET:
            return self.parse_target_directive()
        elif prev_token.type == TokenType.DIRECTIVE_IMPORT:
            return self.parse_import_directive()
        elif prev_token.type == TokenType.DIRECTIVE_REQUIRES:
            return self.parse_requires_directive()
        elif prev_token.type == TokenType.DIRECTIVE_PLUGIN:
            return self.parse_plugin_directive()
        elif prev_token.type == TokenType.DIRECTIVE_DEFINE:
            return self.parse_define_directive()
        else:
            raise ParseError(f"Unexpected directive: {prev_token.lexeme}", prev_token)

    def parse_target_directive(self) -> TargetDirective:
        self.consume(TokenType.NEWLINE, "Expected newline after #target")
        # Allow both indented and non-indented target fields
    ...
    def parse_import_directive(self) -> ImportDirective:
        ...
        # Allow both indented and non-indented import paths
        if self.check(TokenType.INDENT):
            ...
            # Non-indented format - handle multiple import lines\r\n            # Parse the first import line\r\n            module_parts = []\r\n            while not self.check(TokenType.NEWLINE) and not self.check(TokenType.EOF):\r\n                if self.check(TokenType.IDENTIFIER):\r\n                    path_token = self.consume(TokenType.IDENTIFIER, " Expected module path\)\r\n module_parts.append(path_token.lexeme)\r\n if self.match(TokenType.DOUBLE_COLON):\r\n module_parts.append(\::\)\r\n if self.check(TokenType.NEWLINE) or self.check(TokenType.EOF):\r\n break\r\n module_path = \\.join(module_parts)
            imports = []
            while not self.check(TokenType.NEWLINE) and not self.check(TokenType.EOF):
                if self.check(TokenType.IDENTIFIER):
                    path_token = self.consume(TokenType.IDENTIFIER, "Expected module path")
                    imports.append(path_token.lexeme)
                if self.match(TokenType.DOUBLE_COLON):
                    continue  # Allow std::core::cli style
                if self.check(TokenType.NEWLINE) or self.check(TokenType.EOF):
                    break
            self.consume(TokenType.NEWLINE, "Expected newline after import")
            # For now, just use the first import as the module path
            module_path = imports[0] if imports else ""
            alias = None
    ...
    def parse_statement(self) -> Statement:
        if self.match(TokenType.KEYWORD_IF):
            ...
            # Skip dedent tokens at statement level
            self.advance()
            return self.parse_statement()
        elif self.check(TokenType.EOF):
            # End of file, return a dummy statement or handle gracefully
            raise ParseError("Unexpected end of file", self.peek())
        else:
    ...
    def parse_primary_expression(self) -> Expression:
        ...
        if self.check(TokenType.NEWLINE):
            # Handle empty expressions at statement level
            raise ParseError("Expected expression, got newline", self.peek())
        token = self.peek()
    ...
    def parse_base_type(self) -> TypeExpr:
        ...
        if self.match_any(
            ...
            return BasicType(name=self.previous().lexeme)
        ...
        if self.check(TokenType.IDENTIFIER):
            ...
            if generics:
                return NamedType(name=name, generics=generics)
            else:
